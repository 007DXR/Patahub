    ...
    useEffect(() => {
        getRCDByRepoID(parseInt(repoId)).then(async (data, err) => {
            ...
            data = data.map(async (RCD) => {
                let resultImage;
                let codesetLink, datasetLink;
                await getResultLink(RCD.result_id).then((data, err) => {
                    if (err) throw err;
                    resultImage = data;
                });
                await getCodesetLinkByID(RCD.codeset_id).then((data, err) => {
                    if (err) throw err;
                    codesetLink = data;
                });
                await getDatasetLinkByID(RCD.dataset_id).then((data, err) => {
                    if (err) throw err;
                    datasetLink = data;
                });
                return {
                    ...
                    codeLink: codesetLink+RCD.code_link,
                    dataLink: datasetLink+RCD.data_link,
                    rcdId: RCD.rcd_id
                };
            })
